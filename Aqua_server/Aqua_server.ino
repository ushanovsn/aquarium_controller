...
// количество датчиков с коэффицентами калибровки (коэф. и адреса которых описываем в setup)
...
float hTemp = 0.15;            // гистерезис для регулировки температуры
  ...
  if (millis() - timerForTemp >= periodMeasure) {
    ...
    if (!((curLighting < 255) && (curLighting > 0))) {
      ...
      if (avrCurTemp > 0) {        
        ...
        // обогреватель был включен и темп еще не достигла заданной
        if ((lastStateHeat) && (avrCurTemp < (needTemp))) {
          needHeat = true;
        }        
